def Palin(x):
    ...
    def toChars(x):
        ...
        return letterstring
    if len(x) <= 1:
        print("Preparing to return 'True' from Palin from the base case...")
        return True
    else:
        result = (x[0] == x[-1] and Palin(x[1:-1]))
        print("About to return result", result, "from Palin with arguement", x, "...")
